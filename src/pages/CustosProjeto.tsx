  ...
  return (
    <div className="space-y-6 font-work-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Custos de Projeto</h1>
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading ? (
          ...
          filtered.map((p) => (
            <div key={p.id} onClick={() => navigate(`/financeiro/custos-projeto/${p.id}`)} className="card p-6 flex flex-col md:flex-row items-center gap-6 hover:border-lumos-yellow/30 cursor-pointer group bg-[#2a2a2a]">
              <div className="flex-1 w-full">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-lg font-bold text-white group-hover:text-lumos-yellow transition-colors">{p.project_name}</h3>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 w-full md:w-auto border-t md:border-t-0 md:border-l border-lumos-border pt-4 md:pt-0 md:pl-8">
                <div><p className="text-[10px] font-bold text-lumos-text-secondary uppercase">Venda</p><p className="text-sm font-bold text-white">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(p.totalAmount)}</p></div>
                <div><p className="text-[10px] font-bold text-lumos-text-secondary uppercase">Custos</p><p className="text-sm font-bold text-white">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(p.totalCosts)}</p></div>
